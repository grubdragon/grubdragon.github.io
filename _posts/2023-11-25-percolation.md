...
Our true battle is alignment of mental rewards with where we want our attention directed. You see, we tend to play this algorithm in our heads, where the push of our attention is inversely proportional to the rewards we achieve by doing it, roughly exploration/exploitation tradeoffs of the reinforcement learning world.[^rl] We want the algorithm's optimal outcome to favour learning. 

This is where textbook percolation comes into play because *that* is one important downstream measure of how well we are aligning the outcome to our end goal(more like an integration over time of the alignment). While its not perfect, it should capture an overall sense of what we want - "the more I learn, the better it is and the better I feel". That is the holy grail, which is often unachievable but not hopeless to vie upon - we have a few hacks.
